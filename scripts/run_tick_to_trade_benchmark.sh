...

{
  ...
  ninja -C cmake-build-release -t commands tick_to_trade_benchmark
} | tee "$METADATA_OUTPUT"

RUN_ARGS=(--scenario all --warmup 100000 --samples "$SAMPLES" --output-dir "$OUTPUT_DIR")
if command -v taskset >/dev/null 2>&1; then
  taskset -c "$CPU_ID" ./cmake-build-release/tick_to_trade_benchmark \
    "${RUN_ARGS[@]}" | tee -a "$METADATA_OUTPUT"
else
  echo "warning: taskset is unavailable; benchmark thread is not pinned" >&2
  ./cmake-build-release/tick_to_trade_benchmark \
    "${RUN_ARGS[@]}" | tee -a "$METADATA_OUTPUT"
fi

for SCENARIO in maker_book_to_first_order taker_trade_to_order; do
  python3 scripts/summarize_latency.py \
    "$OUTPUT_DIR/ttt_${SCENARIO}.csv" \
    --column corrected_ns | tee "$OUTPUT_DIR/ttt_${SCENARIO}.summary.txt"
done
