...
namespace Trading {
  ...
  auto TradeEngine::sendClientRequest(const Exchange::MEClientRequest *client_request) noexcept -> void {
    ...
    *next_write = std::move(*client_request);
    ...
    if (order_publish_observer_ != nullptr) {
      order_publish_observer_(Common::monotonicRawNanos());
    }
#endif
    outgoing_ogw_requests_->updateWriteIndex();
#if defined(LLT_BENCHMARK_MODE)
    if (ttt_measurement_active_) {
      const auto end = Common::readTSC();
      ttt_recorder_->record(ttt_start_, end);
      ttt_measurement_active_ = false;
      ttt_endpoint_reached_ = true;
    }
#endif
    TTT_MEASURE(T10_TradeEngine_LFQueue_write, logger_);
  }

  auto TradeEngine::processMarketUpdate(const Exchange::MEMarketUpdate *market_update) noexcept -> void {
#if !defined(LLT_BENCHMARK_MODE)
    logger_.log("%:% %() % Processing %\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str_),
                market_update->toString().c_str());
#endif
#if defined(LLT_BENCHMARK_MODE)
    ASSERT(market_update->ticker_id_ < ticker_order_book_.size(), "Unknown ticker-id on market update.");
#else
...
#if defined(LLT_BENCHMARK_MODE)
  auto TradeEngine::processMarketUpdateForBenchmark(const Exchange::MEMarketUpdate *market_update,
                                                     Common::LatencyRecorder *recorder) noexcept -> bool {
    ttt_recorder_ = recorder;
    ttt_endpoint_reached_ = false;
    ttt_measurement_active_ = true;
    ttt_start_ = Common::readTSC();
    processMarketUpdate(market_update);
    ttt_measurement_active_ = false;
    ttt_recorder_ = nullptr;
    return ttt_endpoint_reached_;
  }
#endif
...
#if defined(LLT_TICK_TO_KERNEL_SEND_STAGE_DIAGNOSTIC)
        if (market_dequeue_observer_ != nullptr) {
          market_dequeue_observer_(Common::monotonicRawNanos());
        }
#endif
