...
namespace Trading {
  MarketDataConsumer::MarketDataConsumer(Common::ClientId client_id, Exchange::MEMarketUpdateLFQueue *market_updates,
                                         ...
                                         int core_id)
      : incoming_md_updates_(market_updates), run_(false), core_id_(core_id),
      ...
      for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index_; i += sizeof(Exchange::MDPMarketUpdate)) {
        ...
        if (UNLIKELY(in_recovery_)) {
          ...
          *next_write = std::move(request->me_market_update_);
          ...
          if (incremental_receive_observer_ != nullptr) {
            incremental_receive_observer_(*request, socket->benchmark_recv_complete_ns_);
          }
#endif
          incoming_md_updates_->updateWriteIndex();
#if defined(LLT_TICK_TO_KERNEL_SEND_BENCHMARK)
          if (incremental_publish_observer_ != nullptr) {
            incremental_publish_observer_(request->seq_num_);
          }
#endif
