...
namespace Exchange {
  MEOrderBook::MEOrderBook(TickerId ticker_id, Logger *logger, MatchingEngine *matching_engine)
    ...
    // The benchmark uses deliberately reduced compile-time capacities, so it
    // can initialize and pre-fault the lookup table without the production
    // configuration's multi-gigabyte footprint.
    for (auto& client_orders : cid_oid_to_order_) {
      client_orders.fill(nullptr);
    }
#endif
  }
