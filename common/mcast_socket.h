...
#ifndef LLT_MCAST_BUFFER_SIZE
#define LLT_MCAST_BUFFER_SIZE (64 * 1024 * 1024)
#endif
  constexpr size_t McastBufferSize = LLT_MCAST_BUFFER_SIZE;
  ...
  struct McastSocket {
    ...
    /// Benchmark-only software receive boundary. This is captured immediately
    /// after recv() returns a positive byte count, before decoding/callbacks.
    std::uint64_t benchmark_recv_complete_ns_ = 0;
#endif
